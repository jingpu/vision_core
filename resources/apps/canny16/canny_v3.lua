...
function convertToIllum( I_0 , I_1 , I_2, R, in1 )
   return im(x,y) : cropSame , uint16 clip( (orion.sum( in1(x,y)[0]*I_0 , in1(x,y)[1]*I_1 , in1(x,y)[2]*I_2 )) >> R , 0 , 255 ) end 
end
...
function sobel( in0 )
   ...
   local im in1M(x,y) : cropNone , int16 clip( ( in1x(x,y)*in1x(x,y) + in1y(x,y)*in1y(x,y) ) >> 8 , -127, 127 )   end -- Sx^2 + Sy^2
   return im(x,y) : cropSame, int8[3] { in1x(x,y) , in1y(x,y) , in1M(x,y) } end
end
...
function  nms(  T , P1 , P2 , in1 )
   ...
   im in1(x,y) : cropNone ,  int16[3] ( { in1(x,y)[0] , in1(x,y)[1]<<T , in1(x,y)[2]  }) end
...
function convertToPeakImage2( in1P )
   --return im(x,y) : uint8[3] ( { if in1P(x,y) then 255 else 0 end ,  if in1H(x,y) then 255 else 0 end , 0 }) end
1   --return im(x,y) : uint8[3]   ({ in1(x,y)[0] ,  in1(x,y)[1] , 0 }) end
   return im(x,y) : uint8  if in1P(x,y) then 255 else 0 end end
end
...
function canny(in1)
   ...
   in1 = convertToIllum( I_0 , I_1 , I_2 , R, in1 )            -- Convert to illuminance, Implied up shift by 8 bits 
